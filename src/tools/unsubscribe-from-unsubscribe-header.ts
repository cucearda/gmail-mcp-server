...
// Step messages
const MSG_BROWSER_LAUNCH_ATTEMPT = (headless: boolean) => `Attempting to launch browser (headless: ${headless})`;
const MSG_BROWSER_LAUNCH_SUCCESS = 'Browser launched successfully';
const MSG_BROWSER_LAUNCH_FAILED = (error: string) => `Failed to launch browser: ${error}`;
const MSG_NAVIGATED_TO = (url: string) => `Navigated to ${url}`;
const MSG_NAVIGATION_FAILED = (error: string) => `Failed to navigate: ${error}`;
const MSG_BUTTON_FOUND_BY_SELECTOR = (selector: string) => `Found button using selector: ${selector}`;
const MSG_BUTTON_FOUND_BY_TEXT = (text: string) => `Found button by text: "${text}"`;
const MSG_BUTTON_CLICK_FAILED = (error: string) => `Failed to click: ${error}`;
const MSG_TEXT_BUTTON_CLICK_FAILED = (error: string) => `Failed to click text-based button: ${error}`;
...
const MSG_EMAIL_SENT = (email: string) => `Sent unsubscribe email to ${email}`;
const MSG_EMAIL_SEND_FAILED = (email: string, error: string) => `Failed to send unsubscribe email to ${email}: ${error}`;
...
const MSG_ERROR_PREFIX = (error: string) => `Error: ${error}`;

// Error messages
const ERROR_NO_VALID_LINKS = 'No valid unsubscribe links found in List-Unsubscribe header';

// Debug messages
const DEBUG_LAUNCH_BROWSER = (headless: boolean) => `[DEBUG] About to launch Puppeteer browser (headless: ${headless})`;
const DEBUG_CHROME_EXECUTABLE = (path: string) => `[DEBUG] Found Chrome executable at: ${path}`;
const DEBUG_CHROME_PATH_ERROR = (error: string) => `[DEBUG] Could not determine Puppeteer Chrome path: ${error}`;
const DEBUG_SYSTEM_CHROME = (path: string) => `[DEBUG] Found system Chrome at: ${path}`;
